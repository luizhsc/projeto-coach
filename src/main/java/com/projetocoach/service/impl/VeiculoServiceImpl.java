...
@Service public class VeiculoServiceImpl implements VeiculoService {

    @Autowired private final VeiculoRepository veiculoRepository;

    @Autowired private final VeiculoMapper mapper;
    ...
    @Override public Page<VeiculoDto> findAll(Pageable pageable) {
        return veiculoRepository.findAll(pageable).map(mapper::toDto);
    }

    @Override public VeiculoDto findById(Long id) {
        return mapper.toDto(veiculoRepository.findById(id).orElseThrow(VeiculoException::new));
    }

    @Override public VeiculoDto save(VeiculoDto veiculoDto) {
        return mapper.toDto(veiculoRepository.save(mapper.toModel(veiculoDto)));
    }

    @Override public VeiculoDto update(VeiculoDto veiculoDto) {
        return mapper.toDto(veiculoRepository.save(mapper.toModel(veiculoDto)));
    }
